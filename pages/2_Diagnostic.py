...
<style>
  ...
  .yc-step {{
    display:flex; gap:.6rem; align-items:center; margin:.4rem 0 1rem;
...
if submitted:
    ...
    if missing:
        ...
        try:
            ...
            st.session_state["last_case_id"] = case_id  # 與結果頁/管理頁相容
            # 用旗標 + rerun，避免在 form 回調中直接 switch 失效
            st.session_state["__go_result__"] = True
            st.rerun()
        except Exception as e:
