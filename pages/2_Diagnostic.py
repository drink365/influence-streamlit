...
from src.domain.tax_rules import EstateTaxCalculator, TaxConstants

st.set_page_config(page_title="診斷", page_icon="🧮", layout="wide")

st.title("🧮 傳承風險診斷（estate 稅制對齊版）")
...
with st.form("diag"):
    ...
    with col2:
        st.markdown("#### 扣除與家庭狀況（對齊 estate 程式）")
        ...
        other_dependents = st.number_input("其他受扶養者（兄弟姊妹/祖父母，每人 56 萬）", min_value=0, step=1, value=0)
...
if submitted:
    ...
    calc = EstateTaxCalculator(TaxConstants())
