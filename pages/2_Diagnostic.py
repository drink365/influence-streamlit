...
# 純字串 CSS（避免 f-string 與大括號衝突）
st.markdown(
    """
<style>
  .yc-hero { background:linear-gradient(180deg,#F7F7F8 0%,#FFFFFF 100%);
             border:1px solid rgba(0,0,0,0.04); border-radius:20px;
             padding:24px 28px; box-shadow:0 8px 30px rgba(0,0,0,0.06); }
  .yc-badge { display:inline-block; padding:6px 10px; border-radius:999px;
              background:rgba(168,135,22,0.08); color:#A88716; font-size:12px; font-weight:700;
              border:1px solid rgba(168,135,22,0.27); letter-spacing:.3px; }
  ...
  .yc-step { display:flex; gap:.6rem; align-items:center; margin:.4rem 0 1rem;
             color:#374151; font-weight:700; }
  .yc-dot  { width:26px; height:26px; border-radius:999px; background:rgba(189,14,27,0.07);
             border:1px solid rgba(189,14,27,0.33); display:flex; align-items:center;
             justify-content:center; font-size:12px; color:#BD0E1B; }
...
# ---------------- 基本資料（即時 rerun，非 st.form） ----------------
st.markdown('<div class="yc-step"><div class="yc-dot">1</div><div>基本資料</div></div>', unsafe_allow_html=True)
bc1, bc2, bc3 = st.columns(3)
with bc1:
    name = st.text_input("姓名 *", key="diag_name")
with bc2:
    email = st.text_input("Email *", key="diag_email")
with bc3:
    mobile = st.text_input("手機 *", key="diag_mobile")

fc1, fc2, fc3 = st.columns(3)
with fc1:
    marital = st.selectbox("婚姻狀況 *", ["未婚", "已婚", "離婚", "喪偶"], key="diag_marital")
with fc2:
    children = st.number_input("子女人數 *", min_value=0, max_value=10, step=1, value=st.session_state.get("diag_children", 0), key="diag_children")
with fc3:
    heirs_ready = st.selectbox("是否已有接班人選 *", ["尚未明確", "已明確"], key="diag_heirs")
...
with a1:
    equity = st.number_input("公司股權 *", min_value=0, step=10, value=st.session_state.get("diag_equity", 0), key="diag_equity")
with a2:
    real_estate = st.number_input("不動產 *", min_value=0, step=10, value=st.session_state.get("diag_re", 0), key="diag_re")
with a3:
    financial = st.number_input("金融資產 *", min_value=0, step=10, value=st.session_state.get("diag_fin", 0), key="diag_fin", help="現金/存款/基金/股票等")
with a4:
    insurance_cov = st.number_input("既有保單保額 *", min_value=0, step=10, value=st.session_state.get("diag_cov", 0), key="diag_cov")

total_assets = (
    st.session_state.get("diag_equity", 0)
...
agree = st.checkbox(
    ...
    value=st.session_state.get("diag_agree", True),
    key="diag_agree",
)

missing_live = []
if not (name or "").strip():
    missing_live.append("姓名")
if not (email or "").strip():
    missing_live.append("Email")
if not (mobile or "").strip():
    missing_live.append("手機")
if total_assets <= 0:
    missing_live.append("資產盤點")
if not agree:
    missing_live.append("同意聲明")

if missing_live:
    st.markdown(
        "<div class='yc-alert'>尚未完成項目：" + "、".join(missing_live) + "</div>",
        unsafe_allow_html=True,
    )
