...
def page_book():
    ...
    with st.form("book", clear_on_submit=False):
        ...
        if submit:
            ...
            if not phone.strip(): errors.append("手機")
            if not email.strip(): errors.append("Email")

            # 基本格式檢查
            if phone.strip() and not re.fullmatch(r"[0-9+\-\s]{8,}", phone.strip()):
                errors.append("手機格式")
            if email.strip() and not re.fullmatch(r"[^@\s]+@[^@\s]+\.[^@\s]+", email.strip()):
                errors.append("Email 格式")
            ...
            if errors:
                ...
                ok_user, msg_user = send_email_smtp(email.strip(), user_subject, user_html, user_text)

                # 管理者通知（可選）
                admin_to = st.secrets.get("MAIL_TO_ADMIN") if hasattr(st, "secrets") else None
                if admin_to:
                    ...
                    ok_admin, msg_admin = send_email_smtp(admin_to, admin_subject, admin_html, admin_text)
                else:
